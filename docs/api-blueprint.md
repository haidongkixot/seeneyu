...
| v1.2.0 | 2026-04-15 | Added: Practice Ideating toolkit (5 routes: generate, batches CRUD, export, stats), AI Generator bridge (`POST /push-from-ideating`, `GET/POST /collections/[batchId]`), selective publish with requestIds filter, Practice Collections list + detail pages. Schema: `PracticeIdeaBatch` model, `AiContentRequest` +collectionId/sourcePracticeIdeaId/collectionTitle. Micro-sessions feedback: GPT-4o-mini replaces hardcoded templates. Cron: video-poll `*/10 * * * *`. |
| v1.2.1 | 2026-04-15 | Security hardening: auth gate on `/api/micro-sessions`, mobile token 30-day expiry, mobile login rate limit (10/15min), 7 cron routes deny-by-default, upload validator (type + size) on 4 routes, debug endpoint deleted, trial repeat blocked, security headers (X-Frame-Options, CSP, nosniff, Referrer-Policy), JWT maxAge 30 days. Audit report: `docs/security-audit-2026-04-15.md`. |